...
extension GetGoogleAccountModelCollection on Isar {
  IsarCollection<GoogleAccountModel> get googleAccountModels =>
      this.collection();
}

const GoogleAccountModelSchema = CollectionSchema(
  ...
  links: {},
  embeddedSchemas: {},
  getId: _googleAccountModelGetId,
  getLinks: _googleAccountModelGetLinks,
  attach: _googleAccountModelAttach,
  version: '3.1.0+1',
);
...
Id _googleAccountModelGetId(GoogleAccountModel object) {
  return object.id;
}
...
extension GoogleAccountModelByIndex on IsarCollection<GoogleAccountModel> {
  Future<GoogleAccountModel?> getByEmail(String email) {
    return getByIndex(r'email', [email]);
  }

  GoogleAccountModel? getByEmailSync(String email) {
  ...
  Future<List<GoogleAccountModel?>> getAllByEmail(List<String> emailValues) {
    final values = emailValues.map((e) => [e]).toList();
    return getAllByIndex(r'email', values);
  }

  List<GoogleAccountModel?> getAllByEmailSync(List<String> emailValues) {
    final values = emailValues.map((e) => [e]).toList();
  ...
  Future<Id> putByEmail(GoogleAccountModel object) {
    return putByIndex(r'email', object);
  }

  Id putByEmailSync(GoogleAccountModel object, {bool saveLinks = true}) {
    return putByIndexSync(r'email', object, saveLinks: saveLinks);
  }

  Future<List<Id>> putAllByEmail(List<GoogleAccountModel> objects) {
...
extension GoogleAccountModelQueryWhereSort
    on QueryBuilder<GoogleAccountModel, GoogleAccountModel, QWhere> {
  QueryBuilder<GoogleAccountModel, GoogleAccountModel, QAfterWhere> anyId() {
...
extension GoogleAccountModelQueryLinks
    on QueryBuilder<GoogleAccountModel, GoogleAccountModel, QFilterCondition> {}
...
extension GoogleAccountModelQueryProperty
    on QueryBuilder<GoogleAccountModel, GoogleAccountModel, QQueryProperty> {
  QueryBuilder<GoogleAccountModel, int, QQueryOperations> idProperty() {
    return QueryBuilder.apply(this, (query) {
  ...
  QueryBuilder<GoogleAccountModel, String, QQueryOperations> emailProperty() {
    return QueryBuilder.apply(this, (query) {
      return query.addPropertyName(r'email');
    });
  }

  QueryBuilder<GoogleAccountModel, bool, QQueryOperations> isActiveProperty() {
    return QueryBuilder.apply(this, (query) {
